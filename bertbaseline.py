...
#     input_type_ids=segment_ids)
  ...
  model = modeling.get_bert_model(input_ids, input_mask, segment_ids, config=bert_config)

  model.compile(optimizer='adam',
                loss='sparse_categorical_crossentropy',
                metrics=['accuracy'])
  try:
    print("ATTEMPTING LOAD WEIGHTS")
  ...
  print("FINALHIDDENSHAPE",final_hidden_shape)
  batch_size = final_hidden_shape[0]
  seq_length = final_hidden_shape[1]
  hidden_size = final_hidden_shape[2]
  

#  final_hidden_matrix = tf.keras.layers.Reshape([seq_length * hidden_size])(final_hidden)
#  logits = tf.keras.layers.Dense([2,hidden_size])(final_hidden_matrix)
  logits = tf.keras.layers.Dense(2)(final_hidden)
  print("LOGITSBEFORERES",logits)
#  logits = tf.keras.layers.Reshape([batch_size, seq_length, 2])(logits)
#  logits = tf.keras.layers.Reshape([seq_length, 2])(logits)
  logits = tf.keras.layers.Permute([2, 1])(logits)
...
#  logits = tf.transpose(a=logits, perm=[2, 0, 1])
  ...
  unstacked_logits = tf.unstack(logits, axis=1)
  print("UNSTACKED_LOGITS",unstacked_logits)
...
#   var_list = [output_weights, output_bias, answer_type_output_weights, answer_type_output_bias]
...
#                                       answer_type_output_bias)
  answer_type_logits = tf.keras.layers.Dense(num_answer_types)(answer_type_output_layer)

#  model = tf.keras.models.Model(inputs=[input_ids,input_mask,segment_ids],outputs=[start_logits,end_logits,answer_type_logits])
#  print("MODELSUMMARY",model.summary())
  return model

#  return (start_logits, end_logits, answer_type_logits, var_list)
  ...
  """Returns `model_fn` closure for TPUEstimator."""
    ...
    """The `model_fn` for TPUEstimator."""
        ...
        if init_checkpoint:
            ...
            checkpoint_tf.restore(init_checkpoint).assert_consumed()
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def compute_loss(logits, positions):
                print("LOGITS,POS",logits,positions)
            ...
            def compute_label_loss(logits, labels):
                print("LOGITS,LAB",logits,labels)
    ...
    #       total_loss = tf.math.divide(tf.add_n([start_loss,end_loss,answer_type_loss]), 3.0)
            
            
            def total_loss():
    #         (start_logits, end_logits, answer_type_logits, var_list) = create_model(
    #                                                     bert_config=bert_config,
    #                                                     is_training=is_training,
    #                                                     input_ids=input_ids,
    #                                                     input_mask=input_mask,
    ...
    #                                                     use_one_hot_embeddings=use_one_hot_embeddings)
                start_loss = compute_loss(start_logits, start_positions)
                print("STARTLOSS",start_loss)
                end_loss = compute_loss(end_logits, end_positions)
                print("ENDLOSS",end_loss)
                answer_type_loss = compute_label_loss(answer_type_logits, answer_types)
                print("ANSWER_TYPE_LOSS",answer_type_loss)
                total_loss = tf.add_n([start_loss,end_loss,answer_type_loss])
                #        total_loss = tf.math.divide(tf.add_n([start_loss,end_loss,answer_type_loss]), tf.constant(3.0))
